...
export default function TradesOsLayout({ children }: { children: React.ReactNode }) {
  return (
    <main className="min-h-screen bg-gradient-to-b from-[#07192f] via-[#0f3559] to-[#0a1b30] text-slate-100">
      <header className="sticky top-0 z-40 border-b border-emerald-300/20 bg-[#061426]/90 backdrop-blur">
        <div className="mx-auto max-w-7xl px-6 py-4 md:px-10">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="text-xs uppercase tracking-[0.2em] text-emerald-200">Bid Build</p>
              <p className="text-xs text-slate-300">Run Jobs, Estimate Fast, Win More Work</p>
            </div>
            ...
            <nav className="flex flex-wrap items-center gap-2 text-xs">
              ...
              <Link href="/tradesos/dashboard" className="rounded-full border border-emerald-300/30 bg-emerald-500/10 px-3 py-1.5 font-semibold text-emerald-100 hover:bg-emerald-500/20">
                Dashboard
              </Link>
          ...
          <div className="mt-3 hidden flex-wrap items-center gap-2 text-[11px] uppercase tracking-[0.16em] text-slate-300 md:flex">
            ...
            <span className="rounded-full border border-amber-300/30 bg-amber-500/10 px-3 py-1">Reviews</span>
          </div>
        </div>
      </header>
      {children}
